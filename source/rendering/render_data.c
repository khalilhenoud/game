...
////////////////////////////////////////////////////////////////////////////////
// NOTE: This would benefit from overloading...
...
load_sublevel_mesh_data(
  ...
  const allocator_t *allocator)
{
  assert(sublevel && mesh_data && allocator);

  // 1 texure per mesh for now...
  cvector_setup2(
    &mesh_data->mesh_render_data, get_type_data(mesh_render_data_t));
  cvector_resize(&mesh_data->mesh_render_data, sublevel->meshes.meshes.size);

  cvector_setup2(
    &mesh_data->texture_runtimes, get_type_data(texture_runtime_t));
  cvector_resize(&mesh_data->texture_runtimes, sublevel->meshes.meshes.size);

  cvector_setup2(&mesh_data->texture_ids, get_type_data(uint32_t));
  cvector_resize(&mesh_data->texture_ids, sublevel->meshes.meshes.size);
  memset(
    mesh_data->texture_ids.data,
    0,
    sizeof(uint32_t) * sublevel->meshes.meshes.size);

  for (uint32_t i = 0; i < sublevel->meshes.meshes.size; ++i) {
    mesh_asset_t* mesh = cvector_as(&scene->mesh_repo, i, mesh_t);
    mesh_render_data_t* r_data = cvector_as(
      &mesh_data->mesh_render_data, i, mesh_render_data_t);
  ...
  chashmap_t *assets_map)
{
...

  {
    ...
    load_sublevel_mesh_data(sublevel, &render_data->mesh_data, allocator);
    load_sublevel_light_data(sublevel, &render_data->light_data, allocator);
    return render_data;
  }
}